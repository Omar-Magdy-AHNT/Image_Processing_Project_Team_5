import cv2 as cv
import numpy as np
import time
import serial

def equalize_y_channel(frame):
    # Convert frame to YUV color space
    frame_yuv = cv.cvtColor(frame, cv.COLOR_BGR2YUV)

    # Split the channels
    y, u, v = cv.split(frame_yuv)

    # Equalize the histogram of the Y channel
    y_eq = clahe.apply(y)

    # Merge the channels back
    frame_yuv_eq = cv.merge((y_eq, u, v))

    # Convert YUV to BGR
    frame_output = cv.cvtColor(frame_yuv_eq, cv.COLOR_YUV2BGR)

    return frame_output

# Initialize serial connection to Arduino (optional)
arduino = serial.Serial('COM4', 9600, timeout=1)
time.sleep(2)

# CLAHE setup
clahe = cv.createCLAHE(clipLimit=4.0, tileGridSize=(5, 5))

# Initialize camera
camera = cv.VideoCapture(0)
camera.set(cv.CAP_PROP_FRAME_WIDTH, 640)
camera.set(cv.CAP_PROP_FRAME_HEIGHT, 480)

# FPS setup
prev_time = time.time()
frame_count = 0
fps = 0

# Color bounds for green object (in HSV)
lower_green = np.array([35, 40, 0])
upper_green = np.array([85, 255, 255])

# Morphological kernel
kernel = np.ones((5, 5), np.uint8)

# Control parameters
offset_threshold = 30  # Minimum offset before action
Kpt = 1.5               # Proportional gain
Kpa = 0.5                # Proportional gain
max_pwm = 255          # Max PWM value
min_area = 500        # Minimum contour area to consider 
max_area = 7000      # Minimum contour area to consider

while True:
    ret, frame = camera.read()
    if not ret:
        break

    frame_count += 1
    current_time = time.time()
    elapsed_time = current_time - prev_time

    if elapsed_time >= 1.0:
        fps = frame_count / elapsed_time
        prev_time = current_time
        frame_count = 0

    frame = cv.flip(frame, 1)
    scaled1 = cv.resize(frame, None, fx=0.5, fy=0.5, interpolation=cv.INTER_LINEAR)

    # Enhance image using histogram equalization on Y channel
    scaled = equalize_y_channel(scaled1)

    # Convert to HSV and create mask
    hsv = cv.cvtColor(scaled, cv.COLOR_BGR2HSV)
    mask = cv.inRange(hsv, lower_green, upper_green)
    segmented = cv.bitwise_and(scaled, scaled, mask=mask)

    # Post-process mask
    gray = cv.cvtColor(segmented, cv.COLOR_BGR2GRAY)
    _, binary_mask = cv.threshold(gray, 0, 255, cv.THRESH_BINARY)
    binary_mask = cv.morphologyEx(binary_mask, cv.MORPH_OPEN, kernel)
    binary_mask = cv.morphologyEx(binary_mask, cv.MORPH_CLOSE, kernel)
    
    

    # Find contours
    contours, _ = cv.findContours(binary_mask, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)

    # Filter out small contours
    valid_contours = [cnt for cnt in contours if cv.contourArea(cnt) > min_area]

    if valid_contours:
        largest = max(valid_contours, key=cv.contourArea)
        area = cv.contourArea(largest)
        cv.drawContours(segmented, [largest], -1, (0, 255, 0), 3)

        M = cv.moments(largest)
        if M["m00"] != 0:
            cx = int(M["m10"] / M["m00"])
            cy = int(M["m01"] / M["m00"])
            cv.circle(segmented, (cx, cy), 5, (0, 0, 255), -1)

            frame_center_x = scaled.shape[1] // 2
            horizontal_offset = cx - frame_center_x
            cv.putText(segmented, f"Offset: {horizontal_offset}px", (10, 60), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2, cv.LINE_AA)
            cv.line(segmented, (frame_center_x, 0),
                    (frame_center_x, scaled.shape[0]), (255, 0, 0), 2)

            # === P-Control Logic ===
            if abs(horizontal_offset) > offset_threshold:
                pwm = min(int(Kpt * abs(horizontal_offset)), max_pwm)
                direction = 'L' if horizontal_offset > 0 else 'R'
                command = f"{direction}{pwm:03}\n".encode()
                arduino.write(command)
            elif area > min_area and area < max_area and abs(horizontal_offset) < offset_threshold:
                pwm = min(int(Kpa *(max_area-area)), max_pwm)
                direction = 'F' if area < max_area else 'S'
                command = f"{direction}{pwm:03}\n".encode()
                arduino.write(command)
            else:
                arduino.write(b"S000\n")
            if valid_contours.__len__() < 1:
                arduino.write(b"S000\n")

    # Display FPS
    cv.putText(segmented, f"FPS: {fps:.2f}", (10, 30), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv.LINE_AA)

    # Show windows
    cv.imshow('Segmented', segmented)
    cv.imshow('Mask', binary_mask)
    #cv.imshow('Original', scaled)
    #cv.imshow('og', scaled1)

    if cv.waitKey(1) & 0xFF == ord('q'):
        arduino.write(b"S000\n")
        break

camera.release()
arduino.close()
cv.destroyAllWindows()
